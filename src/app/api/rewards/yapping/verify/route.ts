...
// Function to fetch recent posts from Twitter API
async function fetchUserRecentPosts(twitterHandle: string) {
  ...
  try {
    ...
    // We use Twitter API v2 recent search
    const query = encodeURIComponent(`from:${twitterHandle} @flipfindotfun`);
    const url = `https://api.twitter.com/2/tweets/search/recent?query=${query}&tweet.fields=created_at,text&max_results=10`;
